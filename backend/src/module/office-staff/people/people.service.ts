...
export class PeopleService {
  ...
  async checkUsernameAvailability(username: string) {
    ...
    candidateSet.add(`kk_${cleanUsername}`);
  ...
  async createPerson(dto: CreateStaffPersonDto) {
    if (dto.role !== Role.WORKER && dto.role !== Role.CUSTOMER) {
  ...
  async listPeople(dto: ListStaffPeopleDto) {
    if (
      dto.role &&
  ...
  async getPersonByUsername(username: string) {
    ...
    if (user.role !== Role.WORKER && user.role !== Role.CUSTOMER) {
      throw new NotFoundException(PEOPLE_MESSAGES.PERSON_NOT_FOUND);
    }
  ...
  async updatePerson(id: string, dto: UpdateStaffPersonDto) {
